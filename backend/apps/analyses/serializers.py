...
class MoscowScopeResultSerializer(StrictFieldsSerializer):
    ...
    summary = serializers.CharField(min_length=10, max_length=1000, trim_whitespace=True)
    ...
    def validate(self, attrs):
        features = [item for category in self.CATEGORY_FIELDS for item in attrs[category]]
        if not 8 <= len(features) <= 12:
            raise serializers.ValidationError("The result must contain 8 to 12 features.")

        titles = [self._canonical_title(item["title"]) for item in features]
        if len(titles) != len(set(titles)):
            raise serializers.ValidationError("Feature titles must be unique across categories.")
        return attrs
...
class MoscowScopeAnalysisSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            "id", "idea_id", "summary", "must_have", "should_have", "could_have",
            "wont_have", "prompt_version", "provider", "model_name", "created_at", "updated_at",
        )
